...
#
# The zsh comparison arms are gone with the scripts they measured. The figures they
# produced are recorded in docs/rust-rewrite/tasks/picker/01-project-picker-source.md
# and in FINAL-REVIEW.md; `git show 7731a62^:scripts/project-picker-source.zsh` restores
# the script if the comparison ever has to be re-run.
...
bench "rust: --version (startup floor)" "$binary" --version

# With a query of two or more characters the flow shells out to `zoxide query`, which
# costs about 8 ms on its own. It dominates the comparison, so it is reported
